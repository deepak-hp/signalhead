...
WORK="${TMPDIR:-/tmp}/aitl-linux-check"
...
echo
echo "=== CLI smoke test ==="
"$NODE" src/cli.js start --no-overlay --port 4799 >/tmp/aitl-linux.log 2>&1 &
SRV=$!
...
AITL_PORT=4799 "$NODE" src/cli.js set busy --agent linux --session smoke --detail "checking"
AITL_PORT=4799 "$NODE" src/cli.js fuel 55 --agent linux --session smoke --label "plan quota"
AITL_PORT=4799 "$NODE" src/cli.js status
AITL_PORT=4799 "$NODE" src/cli.js doctor | tail -n 20
AITL_PORT=4799 "$NODE" src/cli.js stop
kill $SRV 2>/dev/null
